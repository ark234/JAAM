...
userModel.create = function create(user) {
	...
	return db.oneOrNone('INSERT INTO users (username, email, password_digest) VALUES ($1, $2, $3) RETURNING *;', [
		user.username,
		user.email,
		passwordDigest
	]);
...
userModel.updateAccount = (req, res, next) => {
	...
	db.one('UPDATE users SET email = $1 WHERE id = $2 RETURNING id;', [req.body.user.email, req.params.userId])
		.then(data => {
...
userModel.deleteAccount = (req, res, next) => {
	...
	db.none('DELETE FROM users WHERE id = $1;', [req.params.userId])
		.then(() => {
			next();
		})
		.catch(error => {
